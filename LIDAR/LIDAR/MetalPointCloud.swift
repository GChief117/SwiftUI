...
final class CoordinatorPointCloud: MTKCoordinator {
    ...
    //changed the float matrix to get the full screen view of the point cloud
    func calcCurrentPMVMatrix(viewSize: CGSize) -> matrix_float4x4 {
        let projection: matrix_float4x4 = makePerspectiveMatrixProjection(fovyRadians: Float.pi / 3.75,
                                                                          aspect:
...
//        let rotationMatrix: matrix_float4x4 = matrix_float4x4(cameraRotation)
        // gets rid of the random rotation and zooms
//        let pmv = projection * rotationMatrix * translationCamera * translationOrig * orientationOrig
        let pmv = projection * translationOrig * orientationOrig
        return pmv
    }

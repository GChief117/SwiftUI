...
final class ARProvider: ARDataReceiver, ObservableObject {
    ...
    let origDepthWidth = 1920
    let origDepthHeight = 1440
   ...
   // func pause() {
    ...
    init() {
        do {
            ...
            destDepthTexture = ARProvider.createTexture(metalDevice: metalDevice, width: origDepthWidth, height: origDepthHeight,
                                                        usage: [.shaderRead, .shaderWrite], pixelFormat: .r32Float)
            destConfTexture = ARProvider.createTexture(metalDevice: metalDevice, width: origDepthWidth, height: origDepthHeight,
